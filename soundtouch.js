 ...
 * @param {any} filter
 * @param {CallableFunction} sourcePositionCallback
...
class PitchShifter {
    constructor(context, buffer, bufferSize, onEnd = noop) {
        ...
        this._filter = new SimpleFilter(this.source, this._soundtouch, onEnd)
    ...
    getChannelData(channel) {
        if (this.source && this.source.buffer) {
            return this.source.buffer.getChannelData(channel)
        }
        return null
    }
    seekToMs(milliseconds) {
        if (milliseconds < 0) {
            milliseconds = 0
        } else if (milliseconds > this.duration * 1000) {
            milliseconds = this.duration * 1000
        }

        const newPosition = Math.floor((milliseconds / 1000) * this.sampleRate)
        this._filter.sourcePosition = newPosition
        this.sourcePosition = newPosition
        this.timePlayed = this.sourcePosition / this.sampleRate

        this.listeners.forEach((listener) => {
            if (listener.name === 'seek') {
                listener.cb(this.formattedTimePlayed)
            }
        })
    }
...
class PitchShifter2 {
    constructor(context, bufferSize, inputNode = null, onEnd = noop) {
        this._soundtouch = new SoundTouch()
        this._filter = null
        this._node = null
        ...
        if (this._node) {
            this._node.onaudioprocess = (event) => {
                let left = event.outputBuffer.getChannelData(0)
                let right = event.outputBuffer.getChannelData(1)
                let framesExtracted = this._filter.extract(left, bufferSize)
                this.sourcePosition = this._filter.sourcePosition
                this.timePlayed = this.sourcePosition / this.sampleRate
                ...
                if (this.sourcePosition >= this.duration * this.sampleRate) {
                    onEnd()
                }

                if (this._filter.inputBuffer.frameCount < bufferSize) {
                    this._filter.onEnd()
                }
            }
        }
    ...
    setInputNode(inputNode) {
        this._filter = new SimpleFilter(inputNode, this._soundtouch)
        this._node = inputNode.context.createScriptProcessor(this.bufferSize, 2, 2)
        this._node.connect(inputNode.context.destination)
    }
    ...
    set percentagePlayed(perc) {
        this.sourcePosition = parseInt(perc * this.duration * this.sampleRate)
        this.timePlayed = this.sourcePosition / this.sampleRate
        this._filter.sourcePosition = this.sourcePosition
    }
...
export { AbstractFifoSamplePipe, PitchShifter, PitchShifter2, RateTransposer, SimpleFilter, SoundTouch, Stretch, WebAudioBufferSource, getWebAudioNode }
